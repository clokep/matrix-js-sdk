    ...
    private async processSyncResponse(syncEventData: ISyncStateData, data: ISyncResponse): Promise<void> {
        ...
        await utils.promiseMapSeries(joinRooms, async (joinObj) => {
            ...
            if (joinObj.unread_notifications) {
                ...
                if (joinObj.unread_thread_notifications) {
                    for (const threadId in joinObj.unread_thread_notifications) {
                        room.getThread(threadId).setUnreadNotificationCount(
                            NotificationCountType.Total,
                            joinObj.unread_thread_notifications[threadId].notification_count,
                        );
                    }
                }
